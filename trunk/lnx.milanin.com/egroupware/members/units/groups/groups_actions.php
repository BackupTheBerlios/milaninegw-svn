<?php
	...
	// Actions to perform on the ".tbl_prefix."groups.screen
			...
			switch($_REQUEST['action']) {
				...
				case "group:create":
										if (isset($_POST['name']) && logged_on) {
											...
											db_query("insert into ".tbl_prefix."groups set
														name = '$name',
														owner = $ident");
				...
				case "group:edit":
										if (
												...
												db_query("update ".tbl_prefix."groups.set name = '$name'
																		where owner = $ident
																		and ident = $groupid");
												unset($_SESSION['groups_cache']);
				...
				case "group:delete":
										if (isset($_POST['groupid']) && logged_on) {
											...
											$ident = (int) $_SESSION['userid'];
											db_query("delete from ".tbl_prefix."groups where ident = $groupid and owner = $ident");
											if (db_affected_rows() > 0) {
												db_query("delete from ".tbl_prefix."group_membership where group_id = $groupid");
											}
											unset($_SESSION['groups_cache']);
										} else {
											// var_export($_POST);
				...
				case "group:addmember":
										if (isset($_POST['groupid']) && logged_on) {
											$groupid = (int) $_POST['groupid'];
											$ident = (int) $_SESSION['userid'];
											$exists = db_query("select * from ".tbl_prefix."groups where ident = $groupid and owner = $ident");
											if (sizeof($exists) > 0) {
												if (sizeof($_POST['friends']) > 0) {
													foreach($_POST['friends'] as $newmember) {
														$newmember = (int) $newmember;
														$exists = db_query("select * from ".tbl_prefix."group_membership
																				where user_id = $newmember
																				and group_id = $groupid");
														if (sizeof($exists) < 1) {
															db_query("insert into ".tbl_prefix."group_membership
											...
											if (sizeof($exists) > 0) {
												if (sizeof($_POST['members']) > 0) {
													foreach($_POST['members'] as $newmember) {
														$newmember = (int) $newmember;
														db_query("delete from ".tbl_prefix."group_membership where user_id = $newmember
																								and group_id = $groupid");
													}
												}
											}
