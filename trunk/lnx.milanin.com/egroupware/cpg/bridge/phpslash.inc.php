...
// ------------------------------------------------------------------------- //
//
//  phpslash Integration for Coppermine
//
// ------------------------------------------------------------------------- //

// ------------------------------------------------------------------------- //
//  Modify the values below according to your phpslash installation
// ------------------------------------------------------------------------- //
...
$pwd = getcwd();
chdir('/home/bestmag/public_html');
...
// $cms_login_tgt =  $_SERVER['PHP_SELF'] . "?login=yes";
$cms_login_tgt = $_PSL['rooturl'] . "/login.php?login=yes";
$cms_logout_tgt = $_PSL['rooturl'] . "/login.php?logout=yes";
...
function cms_authenticate() {
   ...
   $USER_DATA =  psl_get_userdata($USER_DATA);
...
// 
// ----------------------------------------------------
function psl_get_userdata ($USER_DATA) {
   global $sess, $auth, $perm;
 
   // debug('psl_get_userdata::begin', $USER_DATA['user_name']);
   // start out anonymous
   $username = 'Anonymous';
   ...
   // debug("psl_get_userdata::getting username", $username);
   // debug("psl_get_userdata::not getting userdata username", $USER_DATA['user_name']);
   if($USER_DATA['user_name'] != $username) {
      // get user data from coppermine
      // debug("psl_get_userdata::getting userdata for", $username);
      // debug("psl_get_userdata::not getting userdata for", $USER_DATA['user_name']);
      $USER_DATA = psl_get_userdbdata ($username);
   }
   
   if(!$sess->is_registered('USER_DATA')) {
      // debug("psl_get_userdata::session", 'registering');
   ...
   // debug("psl_get_userdata::userdata",$USER_DATA['user_name']);
   return $USER_DATA;
}
...
// 
// ----------------------------------------------------
function psl_get_userdbdata ($username) {
   ...
   if($perm->have_perm('user')) {
      ...
      } elseif( ($USER_DATA = add_phpslash_user($username))) {  
         //psl_debug('user','added');
      } else {
         //psl_debug("user", "not found");
      }
   
      // debug("psl_get_userdbdata::user","logged in");
      $current_time = time();
     ...
     */ 
      ...
      $USER_DATA = psl_get_anondbdata();
   }
   // debug("psl_get_userdbdata::USER_DATA",$USER_DATA);
   return $USER_DATA; 
}
...
// found in phpSlash auth array.
// ----------------------------------------------------
function add_phpslash_user ($username) {
  ...
  //psl_debug("sql", $sql);
   ...
   if ( $db->next_record() ) {
      $USER_DATA = $db->Record;
      //psl_debug("added user", "found");
   } else {
      //psl_debug("added user", "not found");
   }	
...
// 
// ----------------------------------------------------
function psl_get_anondbdata () {
   
   global $sess, $auth, $perm, $CONFIG;
   
   //psl_debug("thispage_id", $thispage_id);
  ...
  /* 
   ...
   define('NUM_URI_BOXES', (int)$USER_DATA['num_URI_upload']);
    */  
   // //psl_debug("psl_get_anondbdata::USER_DATA",$USER_DATA);
   return $USER_DATA; 
}
