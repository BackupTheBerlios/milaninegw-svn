...
$Search = ForceSet(@$_GET["Search"], "");
$Search = urldecode($Search);
$Search = FormatStringForDatabaseInput($Search);
$Sender = ForceIncomingString("Sender", "");
if ($Search != "") {
   ...
   $s->AddLimit(0,10);
   $ResultSet = $Context->Database->Select($Context, $s, "Tools", "AutoComplete", "An error occurred while retrieving autocomplete items.", 0);
   $Name = "";
   $Loop = 1;
   if ($ResultSet) {
      while ($row = $Context->Database->GetRow($ResultSet)) {
         $Name = FormatStringForDisplay($row["Name"], 1);
         echo("<a id=\"AutoCompleteItem".$Loop."\" class=\"ListOption".(($Loop == 1)?"On":"Off")."\" href=\"javascript:".$Sender.".ClickItem('AutoCompleteItem".$Loop."');\" onmouseover=\"".$Sender.".MouseOverAutoCompleteItem(this);\">".$Name."</a>");
         $Loop++;
      }
   }
}
