...
class SqlBuilder {
   ...
   var $Context;
	...
	function AddJoin($NewTable, $NewTableAlias, $NewTableField, $ExistingAlias, $ExistingField, $JoinMethod,$NewTablePrefix="LUM_") {
		$this->Joins .= $JoinMethod." ".$NewTablePrefix.$NewTable." ".$NewTableAlias
	...
	function AddSelect($Field, $TableAlias = "", $FieldAlias = "", $Function = "", $FunctionParameters = "", $GroupByThisField = "0") {
		if (is_array($Field)) {
			for ($i = 0; $i < count($Field); $i++) {
                                //print_r($Field);
                                if (is_array($Field[$i])) {
                                  $this->AddSelect($Field[$i][0],$TableAlias,$Field[$i][1] , "", "", $GroupByThisField);
                                } else {
...
// 		echo "<!--[".$sReturn."-->";
	...
	function SetMainTable($TableName, $TableAlias = "",$TablePrefix="LUM_") {
		$this->MainTable = array("TableName" => $TablePrefix.$TableName, "TableAlias" => $TableAlias);
	}
	...
	function WriteDebug($String, $Color = "") {
