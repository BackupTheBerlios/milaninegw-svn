...
class Validator {
	...
	function Validate() {
		...
		if($this->isRequired) {
			...
			if ($ForcedValue == "") {
				$this->isValid = 0;
				$this->Context->WarningCollector->Add($this->Context->GetDefinition("ErrRequiredInputStart").$this->InputName.$this->Context->GetDefinition("ErrRequiredInputEnd"));
			}
		}
		...
		if (($this->MaxLength > 0) && (strlen($this->Value) > $this->MaxLength)) {
			...
			$this->isValid = 0;
			$this->Context->WarningCollector->Add($this->InputName.$this->Context->GetDefinition("ErrInputLengthMiddle").$CharsToLong.$this->Context->GetDefinition("ErrInputLengthEnd"));
		}
		return $this->isValid;
	}
