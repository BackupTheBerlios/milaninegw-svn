...
Description: Allows users to save little bits of information for easy posting
Version: 1.0
...
if ($Context->SelfUrl == "account.php") {
   ...
   if ($AccountUserID == $Context->Session->UserID) {
               ...
               if ($this->PostBackAction == "Clipping") {
                  ...
                  $this->Context->Writer->Write("<div class=\"SettingsForm\">
                     ...
                     if ($ClippingID > 0) {
                        ...
                        $this->Context->Writer->Write("<div class=\"Form\" id=\"Clippings\">
                           ".$this->Get_Warnings()."
      ...
      $Panel->AddListItem($Context->GetDefinition("AccountOptions"), $Context->GetDefinition("ManageYourClipboard"), $Context->SelfUrl."?PostBackAction=Clipboard");
...
if (in_array($Context->SelfUrl, array("comments.php", "post.php"))) {
   ...
   $Clippings = $ClippingManager->GetClippingsByUserID($Context->Session->UserID);
   $ClippingCount = $Context->Database->RowCount($Clippings);
   $sReturn = "";
   if ($ClippingCount > 0) {
      $ClipboardSelect = $Context->ObjectFactory->NewObject($Context, "Select");
      ...
      $ClipboardSelect->AddOptionsFromDataSet($Context->Database, $Clippings, "ClippingID", "Label");
      $sReturn = "<dt class=\"ClipboardInputLabel\">".$Context->GetDefinition("CopyFromYourClipboard")."</dt>
      <dd class=\"ClipboardInput\">".$ClipboardSelect->Get()."</dd>";
   }
   $Context->ObjectFactory->AddControlString("DiscussionForm", "GetDiscussionForm", $sReturn);
   $Context->ObjectFactory->AddControlString("DiscussionForm", "GetCommentForm", $sReturn);   
}
?>