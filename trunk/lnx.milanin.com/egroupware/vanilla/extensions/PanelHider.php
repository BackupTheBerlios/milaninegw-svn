...
Description: Allows the control panel to be hidden. This functions correctly with the default Vanilla style.
Version: 1.1
...
if (in_array($Context->SelfUrl, array("account.php", "categories.php", "comments.php", "index.php", "post.php", "search.php", "settings.php")) && $Context->Session->UserID > 0) {
   class PanelHider extends Control {
      ...
      function Render() {
         $this->Context->Writer->Write("<div id=\"HiddenPanel\"><a href=\"javascript:RevealPanel();\">&nbsp;</a></div>
         <div id=\"HidePanel\"><a href=\"javascript:HidePanel();\">&nbsp;</a></div>");
      }
   }
   
   if (@$Head) {
      $Head->AddScript($Context->StyleUrl."panelhider.js");
      if ($Context->Session->User->Setting("HidePanel")) {
         $Head->AddStyleSheet($Context->StyleUrl."panelhider.hidden.css", "screen");
      } else {
         $Head->AddStyleSheet($Context->StyleUrl."panelhider.visible.css", "screen");
      }
