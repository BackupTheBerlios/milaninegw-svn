...
}class fud_smtp
{
	...
	if (!($FUD_OPT_2 & 1073741824)) {
		error_dialog('ERRORE: Impossibile spedire email', 'Il sistema di invio email è stato disabilitato dall&#39;amministratore.');
	}
...
function mail_check()
{
	...
	if (isset($_GET['toi']) && (int)$_GET['toi']) {
		...
		if (!($email = q_singleval("SELECT email FROM phpgw_fud_users WHERE alias='".addslashes(htmlspecialchars($_POST['tx_name']))."' AND (users_opt & 16) > 0"))) {
			error_dialog('ERRORE: Impossibile spedire email', 'Il sistema non è in grado di inviare un&#39;email all&#39;utente '.htmlspecialchars($usr_dst->login).' poichè non è in grado di trovare il suo indirizzo di posta elettronica');
		}
	...
	$destination = '<tr class="RowStyleB"><td nowrap class="GenText">A: '.$name_err.'</td><td width="100%" class="RowStyleA"><input type="text" name="tx_name" value="'.htmlspecialchars($tx_name).'"></td></tr>';
...
<tr class="RowStyleB"><td nowrap class="GenText">Oggetto: <?php echo $sub_err; ?></td><td class="RowStyleA" width="100%"><input type="text" name="tx_subject" value="<?php echo htmlspecialchars($tx_subject); ?>"></td></tr>
<tr class="RowStyleB"><td nowrap class="GenText" valign="top">Messaggio: <?php echo $body_err; ?></td><td width="100%" class="RowStyleA"><textarea name="tx_body" cols=40 rows=20><?php echo htmlspecialchars($tx_body); ?></textarea></td></tr>
