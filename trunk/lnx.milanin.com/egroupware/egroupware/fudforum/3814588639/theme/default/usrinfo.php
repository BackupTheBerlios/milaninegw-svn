...
'.$moderation.'
	...
	if ($u->u_last_post_id) {
		...
		if ($usr->users_opt & 1048576 || !empty($frm_perms[$r[3]])) {
			$last_post = '<tr class="'.alt_var('search_alt','RowStyleA','RowStyleB').'"><td valign="top" nowrap class="GenText">Last Post:</td><td class="GenText"><font class="DateText">'.strftime("%a, %d %B %Y %H:%M", $r[2]).'</font><br /><a href="/egroupware/fudforum/3814588639/index.php?t='.d_thread_view.'&amp;goto='.$r[1].'&amp;'._rsid.'" class="GenLink">'.$r[0].'</a></td></tr>';
		}
	}

	$user_image = ($FUD_OPT_2 & 65536 && $u->user_image && strpos($u->user_image, '://')) ? '<tr class="'.alt_var('search_alt','RowStyleA','RowStyleB').'"><td nowrap valign="top" class="GenText">Image:</td><td class="GenText"><img src="'.$u->user_image.'" /></td></tr>' : '';
	...
	if ($u->users_opt & 1) {
		...
		$email_link = '<tr class="'.alt_var('search_alt','RowStyleA','RowStyleB').'"><td nowrap class="GenText">Email:</td><td class="GenText">[<a href="/egroupware/fudforum/3814588639/index.php?t=email&amp;toi='.$u->id.'&amp;'._rsid.'">Click here to email the user</a>]</td></tr>';
	} else {
		$email_link = '';
	}

	if (($referals = q_singleval('SELECT count(*) FROM phpgw_fud_users WHERE referer_id='.$u->id))) {
		$referals = '<tr class="'.alt_var('search_alt','RowStyleA','RowStyleB').'"><td nowrap class="GenText">Referred Users:</td><td class="GenText"><a href="/egroupware/fudforum/3814588639/index.php?t=list_referers&amp;'._rsid.'">'.$referals.' Members</a></td></tr>';
	} else {
		$referals = '';
	}

	if (_uid && _uid != $u->id && !q_singleval("SELECT id FROM phpgw_fud_buddy WHERE user_id="._uid." AND bud_id=".$u->id)) {
		$buddy = '<tr class="'.alt_var('search_alt','RowStyleA','RowStyleB').'"><td nowrap class="GenText">Buddy:</td><td class="GenText"><a href="/egroupware/fudforum/3814588639/index.php?t=buddy_list&amp;add='.$u->id.'&amp;'._rsid.'" class="GenLink">add to buddy list</a></td></tr>';
	} else {
		$buddy = '';
	}
	...
	if (($polls = q_singleval('SELECT count(*) FROM phpgw_fud_poll p INNER JOIN phpgw_fud_forum f ON p.forum_id=f.id WHERE p.owner='.$u->id.' AND f.cat_id>0 '.($usr->users_opt & 1048576 ? '' : ' AND f.id IN('.implode(',', array_keys($frm_perms)).')')))) {
		$polls = '<tr class="'.alt_var('search_alt','RowStyleA','RowStyleB').'"><td nowrap class="GenText">Polls:</td><td class="GenText"><a href="/egroupware/fudforum/3814588639/index.php?t=polllist&amp;uid='.$u->id.'&amp;'._rsid.'">'.$polls.'</a></td></tr>';
	} else {
		$polls = '';
	}
	...
	if ($u->users_opt & 1024) {
		...
		$gender = '<tr class="'.alt_var('search_alt','RowStyleA','RowStyleB').'"><td nowrap class="GenText">Gender:</td><td class="GenText">Female</td></tr>';
	} else {
		$gender = '';
	}

	$location	= $u->location ? '<tr class="'.alt_var('search_alt','RowStyleA','RowStyleB').'"><td nowrap class="GenText">Location:</td><td class="GenText">'.$u->location.'</td></tr>' : '';
	$occupation	= $u->occupation ? '<tr class="'.alt_var('search_alt','RowStyleA','RowStyleB').'"><td nowrap class="GenText">Occupation:</td><td class="GenText">'.$u->occupation.'</td></tr>' : '';
	$interests	= $u->interests ? '<tr class="'.alt_var('search_alt','RowStyleA','RowStyleB').'"><td nowrap class="GenText">Interests:</td><td class="GenText">'.$u->interests.'</td></tr>' : '';
	$bio		= $u->bio ? '<tr class="'.alt_var('search_alt','RowStyleA','RowStyleB').'"><td nowrap class="GenText">Biography:</td><td class="GenText">'.$u->bio.'</td></tr>' : '';
	...
	$im_icq		= $u->icq ? '<tr class="'.alt_var('search_alt','RowStyleA','RowStyleB').'"><td valign="top" nowrap class="GenText"><a name="icq_msg">ICQ Message Form:</a></td><td class="GenText">
		'.$u->icq.' <img src="http://web.icq.com/whitepages/online?icq='.$u->icq.'&amp;img=5" /><br />
			...
			<tr>
				<td>
					<font face="Arial, Helvetica" size="1">Sender Name:</font><br />
					<input type="text" name="from" value="" size="15" maxlength="40" onfocus="this.select()">
				</td>
				<td>
					<font face="Arial, Helvetica" size="1">Sender Email:</font><br />
					<input type="text" name="fromemail" value="" size="15" maxlength="40" onfocus="this.select()">
				</td>
			</tr>
			<tr>
				<td colspan=2>
					<font face="Arial, Helvetica" size="1">Subject</font><br />
					<input type="text" name="subject" value="" size="32"><br />
					<font face="Arial, Helvetica" size="1">Message</font><br />
	...
	if ($u->bday) {
		...
		$birth_date = '<tr class="'.alt_var('search_alt','RowStyleA','RowStyleB').'"><td nowrap class="GenText">Date Of Birth:</td><td class="GenText">'.$bday['month'].' '.$bday['day'].', '.$bday['year'].'</td></tr>';
	} else {
		$birth_date = '';
	}
...
<tr><th colspan=2 width="100%"><?php echo $u->alias; ?>&#39;s Profile</th></tr>
<tr class="RowStyleA"><td nowrap class="GenText">Date Registered:</td><td width="100%" class="DateText"><?php echo strftime("%B %d, %Y", $u->join_date); ?></td></tr>
<tr class="RowStyleB"><td valign="top" nowrap class="GenText">Post Count:</td><td class="GenText"><?php echo $u->posted_msg_count; ?> Post(s) (<?php echo $avg; ?> average posts per day)<br /><a class="GenLink" href="/egroupware/fudforum/3814588639/index.php?t=showposts&amp;id=<?php echo $u->id; ?>&amp;<?php echo _rsid; ?>">Show all posts by <?php echo $u->alias; ?></a></td></tr>
<?php echo $status; ?>
<?php echo $avatar; ?>
<?php echo $last_post; ?>
<?php echo $polls; ?>
...
<?php echo $im_jabber; ?>
<?php echo $im_affero; ?>
<tr class="RowStyleC"><td nowrap align="right" class="GenText" colspan=2><a class="GenLink" href="/egroupware/fudforum/3814588639/index.php?t=showposts&amp;id=<?php echo $u->id; ?>&amp;<?php echo _rsid; ?>">Show all posts by <?php echo $u->alias; ?></a></td></tr>
</table>
<?php echo $page_stats; ?>
</td></tr></table>
