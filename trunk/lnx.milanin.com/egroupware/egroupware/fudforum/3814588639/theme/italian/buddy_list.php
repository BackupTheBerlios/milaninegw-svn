...
}function alt_var($key)
{
	...
	if (isset($_POST['add_login'])) {
		if (!($buddy_id = q_singleval("SELECT id FROM phpgw_fud_users WHERE alias='".addslashes(htmlspecialchars($_POST['add_login']))."'"))) {
			error_dialog('Impossibile aggiungere l&#39;utente', 'L&#39;utente che hai cercato di inserire nella buddy list non risulta iscritto al forum. Attenzione: il motore di inserimento degli utenti distingue tra MAIUSCOLE e minuscole: controlla con attenzione!');
		}
		if ($buddy_id == _uid) {
			error_dialog('Informazioni', 'Non puoi aggiungere te stesso alla buddy list');
		}
		...
		if (!isset($usr->buddy_list[$buddy_id])) {
			...
			error_dialog('Informazioni', 'Hai già inserito questo utente nella tua buddy list');
		}
	}
	...
	ses_update_status($usr->sid, 'Sfoglia la propria buddy list');
...
<tr class="tab">'.$tabs.'</tr>
	...
	if (($r = @db_rowarr($c))) {
		do {
			$homepage_link = $r[7] ? '<a class="GenLink" href="'.$r[7].'" target="_blank"><img src="/egroupware/fudforum/3814588639/theme/italian/images/homepage.gif" alt="" /></a>' : '';
			if ((!($r[5] & 32768) && $FUD_OPT_2 & 32) || $usr->users_opt & 1048576) {
				$online_status = (($r[8] + $LOGEDIN_TIMEOUT * 60) > __request_timestamp__) ? '<img src="/egroupware/fudforum/3814588639/theme/italian/images/online.gif" title="'.$r[2].' is currently online" alt="'.$r[2].' is currently online" />' : '<img src="/egroupware/fudforum/3814588639/theme/italian/images/offline.gif" title="'.$r[2].' is currently offline" alt="'.$r[2].' is currently offline" />';
			} else {
				$online_status = '';
			}
			...
			if ($r[5] && substr($r[4], 4) == date('md')) {
				...
				$bday_indicator = '<img src="blank.gif" alt="" width=10 height=1 /><img src="/egroupware/fudforum/3814588639/theme/italian/images/bday.gif" alt="" />Oggi '.$r[2].' compie '.$age.' anni!!! AUGURI!!!';
			} else {
				$bday_indicator = '';
			}
	...
	<td width="100%" class="GenText">'.$contact_link.'&nbsp;<font class="SmallText">(<a href="/egroupware/fudforum/3814588639/index.php?t=buddy_list&amp;'._rsid.'&amp;del='.$r[0].'" class="GenLink">rimuovi</a>)</font>&nbsp;'.$bday_indicator.'</td>
	<td align="center">'.$r[6].'</td>
	<td align="center" nowrap>'.strftime("%a, %d %B %Y %H:%M", $r[3]).'</td>
	<td class="GenText" nowrap><a class="GenLink" href="/egroupware/fudforum/3814588639/index.php?t=usrinfo&amp;id='.$r[1].'&amp;'._rsid.'"><img src="/egroupware/fudforum/3814588639/theme/italian/images/msg_about.gif" alt="" /></a>&nbsp;<a href="/egroupware/fudforum/3814588639/index.php?t=showposts&amp;'._rsid.'&amp;id='.$r[1].'" class="GenLink"><img src="/egroupware/fudforum/3814588639/theme/italian/images/show_posts.gif" alt="" /></a> '.$homepage_link.'</td>
</tr>';
		} while (($r = db_rowarr($c)));
		$buddies = '<table border="0" cellspacing="1" cellpadding="2" class="ContentTable">
<tr><th>Status</th><th>Elenco Buddy</th><th nowrap align="center">Numero di messaggi</th><th nowrap align="center">Registrato</th><th nowrap align="center">Azione</th></tr>
'.$buddies.'
</table>';
	}
...
<tr><th nowrap>Aggiungi Buddy</th></tr>
<tr class="RowStyleA">
