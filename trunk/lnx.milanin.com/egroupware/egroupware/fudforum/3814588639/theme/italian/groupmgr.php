...
function draw_tmpl_perm_table($perm, $perms, $names)
{
	...
	foreach ($perms as $k => $v) {
		$str .= ($perm & $v[0]) ? '<td title="'.$names[$k].'" class="permYES">Sì</td>' : '<td title="'.$names[$k].'" class="permNO">No</td>';
	}
	return $str;
}
...
<tr class="RowStyleC"><td colspan=2 align="left"><input type="submit" class="button" name="btn_groupswitch" value="Modifica gruppo"></td></tr>
	...
	$indicator = '<div align="center">Modifica: <b>'.$grp->name.'</b><br>'.$group_resources.'</div>';

	$login_error = '';
	$gr_member = isset($_POST['gr_member']) ? $_POST['gr_member'] : '';
	$find_user = $FUD_OPT_1 & (8388608|4194304) ? '&nbsp;&nbsp;&nbsp;[<a href="javascript://" class="GenLink" onClick="javascript: window_open(\'/egroupware/fudforum/3814588639/index.php?t=pmuserloc&'._rsid.'&js_redr=groupmgr.gr_member&overwrite=1\', \'user_list\', 250,250);">Trova utente</a>]' : '';
	$perm = 0;
	...
	if (isset($_POST['btn_submit'])) {
		...
		if (empty($_POST['edit'])) {
			if (!($usr_id = q_singleval("SELECT id FROM phpgw_fud_users WHERE alias='".addslashes(htmlspecialchars($gr_member))."'"))) {
				$login_error = '<font class="ErrorText">Non esiste utente con login "'.htmlspecialchars($gr_member).'"</font><br />';
			} else if (q_singleval('SELECT id FROM phpgw_fud_group_members WHERE group_id='.$group_id.' AND user_id='.$usr_id)) {
				$login_error = '<font class="ErrorText">L&#39;utente "'.htmlspecialchars($gr_member).'" esiste già all&#39;interno di questo gruppo.</font><br />';
			} else {
...
'p_EDIT'=>'Modifica',
'p_DEL'=>'Cancella',
'p_STICKY'=>'Messaggi toppati',
'p_POLL'=>'Crea sondaggi',
'p_FILE'=>'Allega file',
'p_VOTE'=>'Vota',
'p_RATE'=>'Vota i topic',
'p_SPLIT'=>'Spezza i topic',
'p_LOCK'=>'Chiudi topic',
'p_MOVE'=>'Sposta topic',
'p_SML'=>'Usa smiley',
...
<select name="'.$k.'" class="SmallText">
	...
	<option value="'.$v[0].'"'.$selyes.'>Sì</option>
</select>
</td>';
		} else {
...
<tr class="RowStyleB">'.$perm_select.'</tr>';
	...
	if (!$edit) {
		$member_input = '<tr class="RowStyleA"><td nowrap><b>Utente</b></td><td width="100%" align="left">'.$login_error.'<input type="text" name="gr_member" value="'.$gr_member.'">'.$find_user.'</td></tr>';
		$submit_button = '<input type="submit" class="button" name="btn_submit" value="Aggiungi utente">';
	} else {
		$submit_button = '<input type="submit" class="button" name="btn_cancel" value="Annulla"> 
<input type="submit" class="button" name="btn_submit" value="Aggiorna utente">';
		$member_input = '<tr class="RowStyleA"><td nowrap><b>Utente</b></td><td width="100%" align="left">'.$gr_member.'</td></tr>';
	}
...
<br /><br />
<table border="0" cellspacing="1" cellpadding="2" class="ContentTable">
<tr><th>Utente</th><th colspan="'.$n_perms.'">Permissions <font size="-1">(move mouse over the permission to see permission type)</font></th><th align="center">Azione</th></tr>
'.$group_members_list.'
</table>';
