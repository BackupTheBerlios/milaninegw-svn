...
function path_info_lnk($var, $val)
{
	...
	if (_uid) {
		...
		$subscribed_thr = '&nbsp;| <a class="GenLink" href="/egroupware/fudforum/3814588639/index.php?'.$th_opt.'" title="Abilita il filtro per i topic sottoscritti (mostra solo i post dei topic cui sei iscritto)">Topic sottoscritti '.$s_subt.'</a>';
		$subscribed_frm = '&nbsp;| <a class="GenLink" href="/egroupware/fudforum/3814588639/index.php?'.$frm_opt.'" title="Abilita il filtro per i forum sottoscritti (mostra solo i forum cui sei iscritto)">Forum cui sei iscritto '.$s_subf.'</a>';
		$unread_messages = '&nbsp;| <a class="GenLink" href="/egroupware/fudforum/3814588639/index.php?'.$un_opt.'" title="Abilita il filtro per i messaggi non letti (mostra soli i messaggi non letti)">Messaggi non letti '.$s_unread.'</a>';
	} else {
		$subscribed_thr = $subscribed_frm = $unread_messages = '';
	}

	$todays_posts = '<a class="GenLink" href="/egroupware/fudforum/3814588639/index.php?'.$dt_opt.'" title="Abilita il filtro per i messaggi di oggi (mostra solo i messaggi odierni)">Messaggi di oggi '.$s_today.'</a>';
	$unanswered = '&nbsp;| <a class="GenLink" href="/egroupware/fudforum/3814588639/index.php?'.$rp_opt.'" title="Abilita il filtro per i messaggi senza risposta (mostra solo i messaggi che non hanno reply)">Post senza risposta '.$s_unu.'</a>';
	...
	if (!$message_data) {
		if (isset($_GET['unread'])) {
			...
			$message_data = '<tr><th align=middle>Nessun messaggio</th></tr>';
		}
	}
	...
	if (!$unread_limit && $total > $count) {
		...
		$pager = '<div align="center" class="GenText">[<a href="/egroupware/fudforum/3814588639/index.php?'.$_SERVER['QUERY_STRING'].'" class="GenLink" title="Mostra altri messaggi non letti e mostra i messaggi visibili attualmente come letti">altri messaggi non letti</a>]</div><img src="blank.gif" alt="" height=3 />';
	} else {
		$pager = '';
	}
