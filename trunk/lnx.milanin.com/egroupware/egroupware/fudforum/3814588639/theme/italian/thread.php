...
$unanswered_posts = !$th ? '<b>::</b> <a class="GenLink" href="/egroupware/fudforum/3814588639/index.php?t=selmsg&amp;reply_count=0&amp;'._rsid.'&amp;frm_id='.$frm->id.'" title="Mostra tutti i messaggi che non hanno risposta">Messaggi senza risposta</a>' : '';
	...
	if (!($r = @db_rowarr($result))) {
		...
		$threaded_view = $FUD_OPT_2 & 512 ? '<a href="/egroupware/fudforum/3814588639/index.php?t=threadt&amp;frm_id='.$frm->id.'&amp;'._rsid.'" class="GenLink"><img title="Passa alla visuale di default per questo topic" alt="Passa alla visuale di default per questo topic" src="/egroupware/fudforum/3814588639/theme/italian/images/tree_view.gif" /></a>&nbsp;' : '';
...
<td width=1 class="RowStyleB"><img src="/egroupware/fudforum/3814588639/theme/italian/images/moved.png" title="Questo topic è stato spostato in un altro forum" /></td>
<td class="RowStyleB" colspan="5" align="center" class="GenText"><a href="/egroupware/fudforum/3814588639/index.php?t='.d_thread_view.'&goto='.$r[15].'&'._rsid.'" class="GenLink">'.$r[2].'</a> è stato spostato in <a href="/egroupware/fudforum/3814588639/index.php?t=thread&frm_id='.$r[11].'&'._rsid.'" class="GenLink">{'.$r[12].'</a> dal moderatore</td>
</tr>';
				continue;
			}
			$msg_count = $r[16] + 1;
			...
			if ($msg_count > $ppg && $usr->users_opt & 256) {
				...
				if ($mini_pager_data) {
					$mini_thread_pager = '<font class="SmallText">(<img src="/egroupware/fudforum/3814588639/theme/italian/images/pager.gif" alt="" />'.$mini_pager_data.')</font>';
				} else {
					$mini_thread_pager = '';
				}
			} else {
			...
			$thread_poll_indicator = $r[1] ? 'Sondaggio:&nbsp;' : '';
			$thread_attach_indicator = $r[0] ? '<img src="/egroupware/fudforum/3814588639/theme/italian/images/attachment.gif" alt="" />' : '';
			$thread_icon = $r[3] ? '<img src="images/message_icons/'.$r[3].'" alt="'.$r[3].'" />' : '&nbsp;';
			if ($FUD_OPT_2 & 4096) {
				$rating = $r[17] ? '<img src="/egroupware/fudforum/3814588639/theme/italian/images/'.$r[17].'stars.gif" alt="'.$r[17].'" />' : '';
			} else {
				$rating = '';
			}
			if ($r[18] > 1) {
				$stick_status = $r[18] & 4 ? '<font class="StClr">topped:&nbsp;</font>' : '<font class="AnClr">annuncio:&nbsp;</font>';
			} else {
				$stick_status = '';
			}
			...
			if (_uid && $usr->last_read < $r[10] && $r[10] > $r[20]) {
				if ($r[18] & 1) {
					$thread_read_status = '<img src="/egroupware/fudforum/3814588639/theme/italian/images/unreadlocked.png" width=32 height=32 title="Topic chiuso con messaggi non letti" alt="" />';
				} else {
					$thread_read_status = '<img src="/egroupware/fudforum/3814588639/theme/italian/images/unread.png" width=32 height=32 title="Questo topic contiene messaggi che non hai ancora letto" alt="" />';
				}
				/* do not show 1st unread message link if thread has no replies */
				if ($r[16]) {
					$first_unread_msg_link = '<a href="/egroupware/fudforum/3814588639/index.php?t='.d_thread_view.'&amp;th='.$r[13].'&amp;unread=1&amp;'._rsid.'"><img src="/egroupware/fudforum/3814588639/theme/italian/images/newposts.gif" title="Clicca qui per andare al primo messaggio non letto di questo topic" alt="" /></a>&nbsp;';
				}
			}

			if (!$thread_read_status) {
				if ($r[18] & 1) {
					$thread_read_status = '<img src="/egroupware/fudforum/3814588639/theme/italian/images/readlocked.png" width=32 height=32 title="Questo topic è stato chiuso" alt="" />';
				} else if (!_uid) {
					...
					$thread_read_status = '<img src="/egroupware/fudforum/3814588639/theme/italian/images/read.png" width=32 height=32 title="Questo topic non ha messaggi non letti" alt="" />';
				}
			}
			...
			if ($admin_heading_row) {
				if ($MOD || (($frm->group_cache_opt & 8224) == 8224)) {
					$admin_control_row = '<div class="ModOpt">Opzioni del moderatore: <a href="javascript://" onClick="javascript: window_open(\'/egroupware/fudforum/3814588639/index.php?t=mvthread&amp;'._rsid.'&amp;th='.$r[13].'\', \'th_move\', 300, 400);">Sposta</a> | <a href="/egroupware/fudforum/3814588639/index.php?t=mmod&amp;'._rsid.'&amp;th='.$r[13].'&amp;del='.$r[15].'">Cancella</a></div>';
				} else if ($frm->group_cache_opt & 32) {
					$admin_control_row = '<div class="ModOpt">Opzioni del moderatore: <a href="/egroupware/fudforum/3814588639/index.php?t=mmod&amp;'._rsid.'&amp;th='.$r[13].'&amp;del='.$r[15].'">Cancella</a></div>';
				} else {
					$admin_control_row = '<div class="ModOpt">Opzioni del moderatore: <a href="javascript://" onClick="javascript: window_open(\'/egroupware/fudforum/3814588639/index.php?t=mvthread&amp;'._rsid.'&amp;th='.$r[13].'\', \'th_move\', 300, 400);">Sposta</a></div>';
				}
			} else {
				$admin_control_row = '';
			}
...
<div class="TopBy">Da: '.$first_post_login.' on <font class="DateText">'.strftime("%a, %d %B %Y", $r[4]).'</font></div>'.$admin_control_row.'</td>
<td class="RowStyleB" align="center">'.$r[16].'</td>
<td class="RowStyleB" align="center">'.$r[19].'</td>
<td class="RowStyleC" nowrap><font class="DateText">'.strftime("%a, %d %B %Y %H:%M", $r[10]).'</font><br />Da: '.$user_link.' <a href="/egroupware/fudforum/3814588639/index.php?t='.d_thread_view.'&amp;goto='.$r[9].'&amp;'._rsid.'" class="GenLink"><img src="/egroupware/fudforum/3814588639/theme/italian/images/goto.gif" title="Vai all&#39;ultimo messaggio di questo topic" alt="" /></a></td>
</tr>';
...
<td valign="bottom" align="right" class="GenText"><?php echo $threaded_view; ?><a class="GenLink" href="/egroupware/fudforum/3814588639/index.php?t=post&amp;frm_id=<?php echo $frm->id; ?>&amp;<?php echo _rsid; ?>"><img src="/egroupware/fudforum/3814588639/theme/italian/images/new_thread.gif" alt="Crea un nuovo topic" /></a></td>
</tr></table>
...
<td align="right" class="GenText" valign="bottom" nowrap><?php echo $threaded_view; ?><a class="GenLink" href="/egroupware/fudforum/3814588639/index.php?t=post&amp;frm_id=<?php echo $frm->id; ?>&amp;<?php echo _rsid; ?>"><img src="/egroupware/fudforum/3814588639/theme/italian/images/new_thread.gif" alt="Crea un nuovo topic" /></a></td>
</tr>
</table>
