...
function export_msg_data($m, &$msg_subject, &$msg_body, &$msg_icon, &$msg_smiley_disabled, &$msg_show_sig, &$msg_track, &$msg_to_list, $repl=0)
{
	...
	if (($fldr_size = q_singleval('SELECT SUM(length) FROM phpgw_fud_pmsg WHERE duser_id='._uid)) > $MAX_PMSG_FLDR_SIZE) {
		error_dialog('Non hai più spazio!', 'Le tue cartelle di messaggi privati hanno superato la quota limite di <b>'.$GLOBALS['MAX_PMSG_FLDR_SIZE'].' byte</b>, poichè contengono <b>'.$fldr_size.' byte</b> di messaggi.<br><br><font class="ErrorText">Cancella qualcuno dei vecchi messaggi per liberare dello spazio</font>.');
	}
	is_allowed_user($usr);

	$attach_control_error = null;
	$pm_find_user = $FUD_OPT_1 & (8388608|4194304) ? '[<a href="javascript://" class="GenLink" onClick="javascript: window_open(\'/egroupware/fudforum/3814588639/index.php?t=pmuserloc&amp;'._rsid.'&amp;js_redr=post_form.msg_to_list\',\'user_list\',250,250);">Trova utente</a>]' : '';
	...
	if (!isset($_POST['prev_loaded'])) {
		...
		if (isset($_GET['msg_id']) && ($msg_id = (int)$_GET['msg_id'])) { /* editing a message */
			...
			if (($msg_r = db_sab('SELECT id, post_stamp, ouser_id, subject, length, foff, to_list, icon, attach_cnt, pmsg_opt, ref_msg_id '.(isset($_GET['quote']) ? ', to_list' : '').' FROM phpgw_fud_pmsg WHERE id='.(int)(isset($_GET['quote']) ? $_GET['quote'] : $_GET['forward']).' AND duser_id='._uid))) {
				...
				if ($quote) {
					if ($FUD_OPT_1 & 4096) {
						$msg_body = '[quote title='.htmlspecialchars($msg_r->login).' scritto '.strftime("%a, %d %B %Y %H:%M", $msg_r->post_stamp).']'.$msg_body.'[/quote]';
					} else if ($FUD_OPT_1 & 2048) {
						$msg_body = str_replace('<br>', "\n", 'Quote: '.htmlspecialchars($msg_r->login).' scritto '.strftime("%a, %d %B %Y %H:%M", $msg_r->post_stamp).'<br />----------------------------------------------------<br />'.$msg_body.'<br />----------------------------------------------------<br />');
					} else {
						$msg_body = '<table border="0" align="center" width="90%" cellpadding="3" cellspacing="1"><tr><td class="SmallText"><b>'.htmlspecialchars($msg_r->login).' scritto '.strftime("%a, %d %B %Y %H:%M", $msg_r->post_stamp).'</b></td></tr><tr><td class="quote"><br />'.$msg_body.'<br /></td></tr></table>';
					}
...
<tr><td align="left" class="RowStyleB">'.$apply_spell_changes.'<input type="submit" class="button" name="btn_submit" value="Invia" tabindex="5" onClick="javascript: document.post_form.btn_action.value=\'send\';">&nbsp;<input type="submit" tabindex="4" class="button" value="Anteprima del messaggio" name="preview">&nbsp;'.$spell_check_button.'<input type="submit" class="button" name="btn_draft" value="Salva bozza" onClick="javascript: document.post_form.btn_action.value=\'draft\';"></td></tr>
</table></div><br />';
	} else {
		$preview_message = '';
	}
	...
	$fud_code_icons = $FUD_OPT_1 & 4096? '<tr class="RowStyleA"><td nowrap class="GenText">Strumenti di formattazione:</td><td>
...
<option value="Century" style="font-family:Century">Century</option>
	...
	if ($reply && ($mm = db_sab('SELECT p.*, u.sig, u.alias, u.users_opt, u.posted_msg_count, u.join_date, u.last_visit FROM phpgw_fud_pmsg p INNER JOIN phpgw_fud_users u ON p.ouser_id=u.id WHERE p.duser_id='._uid.' AND p.id='.$reply))) {
		...
		$reference_msg = '<br /><br />
<div align="center">messaggio che stai inoltrando a</div>
...
<table border="0" cellspacing="1" cellpadding="2" class="ContentTable">
<tr><th colspan=2>Crea nuovo topic<a name="ptop"> </a></th></tr>
<tr class="RowStyleB"><td nowrap class="GenText">Utente collegato:</td><td class="GenText" width="100%"><?php echo $usr->login; ?></td></tr>
<tr class="RowStyleB"><td class="GenText">A:</td><td class="GenText"><input type="text" name="msg_to_list" value="<?php echo $msg_to_list; ?>" tabindex="1"> <?php echo $pm_find_user; ?> [<a href="javascript://" onClick="javascript: window_open('/egroupware/fudforum/3814588639/index.php?t=qbud&amp;<?php echo _rsid; ?>&amp;1=1', 'buddy_list',275,300);" class="GenLink">Seleziona dalla Buddy List</a>]<?php echo $to_err; ?></td></tr>
...
<tr class="RowStyleA"><td nowrap valign=top class="GenText">Testo:<br /><br /><?php echo $post_options; ?></td><td><?php echo $message_err; ?><textarea id="txtb" rows="20" cols="65" wrap="virtual" tabindex="3" name="msg_body" onKeyUp="storeCaret(this);" onClick="storeCaret(this);" onSelect="storeCaret(this);"><?php echo $msg_body; ?></textarea></td></tr>
<?php echo $file_attachments; ?>
<tr class="RowStyleB" valign="top">
<td class="GenText">Opzioni:</td>
<td>
<table border=0 cellspacing=0 cellpadding=1>
<tr><td><input type="checkbox" name="msg_track" value="Y"<?php echo $msg_track_check; ?>></td><td class="GenText"><b>Controlla questo messaggio</b></td></tr>
<tr><td>&nbsp;</td><td><font class="SmallText">Notifica (tramite messaggio privato) quando questo messaggio viene letto.</font></td></tr>
<tr><td><input type="checkbox" name="msg_show_sig" value="Y"<?php echo $msg_show_sig_check; ?>></td><td class="GenText"><b>Inserisci signature</b></td></tr>
<tr><td>&nbsp;</td><td><font class="SmallText">Inserisci la signature del mio profilo.</font></td></tr>
<?php echo $disable_smileys; ?>
</table>
</td>
</tr>
<tr class="RowStyleA"><td class="GenText" align="right" colspan=2><input type="submit" tabindex="4" class="button" value="Anteprima del messaggio" name="preview">&nbsp;<?php echo $spell_check_button; ?><input type="submit" class="button" name="btn_draft" value="Salva bozza" onClick="javascript: document.post_form.btn_action.value='draft';">&nbsp;<input type="submit" class="button" name="btn_submit" value="Invia" tabindex="5" onClick="javascript: document.post_form.btn_action.value='send';"></td></tr>
</table>
<?php echo _hs; ?>
